...
export default function ChipStack({ chips, phase, className = '' }: ChipStackProps) {
  ...
  return (
    <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 w-[18px] h-[18px] pointer-events-none ${className}`}>
      {visibleChips.map((chipVal, indexInSlice) => {
        ...
        return (
          <motion.div
            ...
            animate={{ scale: 1, opacity: 1, y: -indexInSlice * 2 }}
      ...
      {hiddenCount > 0 && (
        <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 whitespace-nowrap bg-black/80 px-1.5 py-0.5 rounded-sm border border-white/10 shadow-xl z-[100]">
          <span className="text-[7px] font-black text-white leading-tight">
            +{hiddenCount}
          </span>
        </div>
      )}
